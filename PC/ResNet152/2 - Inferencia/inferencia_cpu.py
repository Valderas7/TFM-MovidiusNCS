...
filas = open('/home/user/T-F-M/resnet152/modelos/labels.txt').read().strip().split("\n") # Divide en filas el documento
...
directorio = "/home/user/T-F-M/resnet152/test"
imagenes = []

# Se carga la red preentrenada:
red = cv2.dnn.readNetFromCaffe('/home/user/T-F-M/resnet152/modelos/resnet-152.prototxt', '/home/user/T-F-M/resnet152/modelos/resnet-152.caffemodel')
...
# Se realiza "mean subtraction" (104, 117, 123) para normalizar las imágenes de entrada
...
for j in range(len(imagenes)):
    # Se especifica el BLOB como entrada de la red y se obtiene la clasificación en la salida de la red:
    blob = cv2.dnn.blobFromImage(imagenes[j], 1, (224, 224), (104, 117, 123))
    red.setInput(blob)
    ...
    for (i, indice) in enumerate(indice):
        ...
        if i == 0:
            ...
            texto = "Clase: {}, {:.2f}%".format(clases[indice+1],resultado[0][indice] * 100)
